...
const Navbar = ({ toggleSidebar }) => {
  return (
    <div className="w-full flex justify-between items-center px-2 gap-4 md:px-10 md:gap-10 lg:px-20 lg:gap-20">
      <GiHamburgerMenu
        className="text-2xl flex md:hidden cursor-pointer"
      ...
      <img
        src={ProfileImg}
        className="w-10 h-10 text-4xl rounded-full text-white bg-blue-600 hover:bg-blue-500 cursor-pointer transition-all duration-300 ease-in-out"
      />
    </div>
  );
};
